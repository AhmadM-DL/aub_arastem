...
def infere_from_model(endpoint, prompt, token):
...
def main():
    ...
    parser.add_argument('-t', '--token', type=str, required=True, help="HF Token")
    ...
    for i, q in enumerate(data[checkpoint+1:]):
        ...
        pred, conf = infere_from_model(args.endpoint, prompt, args.token)
